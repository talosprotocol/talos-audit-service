...
class PostgresAuditStore:
                ...
                cur.execute(
                    ...
                    (
                        ...
                        int(
                            ...
                            or self._parse_ts(getattr(event, "ts", "0"))
                        ),
                        getattr(event, "cursor", "")
                        or self._derive_cursor(
                            getattr(event, "ts", "0"), getattr(event, "event_id")
                        ),
                        ...
                        or (event.meta.get("session_id") if getattr(event, "meta", None) else None),
                        ...
                        or (
                            event.principal.get("id") if getattr(event, "principal", None) else None
                        ),
                        getattr(event, "peer_id", None),
                        getattr(event, "tool", None)
                        or (
                            event.resource.get("type") if getattr(event, "resource", None) else None
                        ),
                        getattr(event, "method", None)
                        or (event.http.get("path") if getattr(event, "http", None) else None),
                        getattr(event, "resource_id", None)
                        or (event.resource.get("id") if getattr(event, "resource", None) else None)
                        or (str(event.resource) if getattr(event, "resource", None) else None),
                        ...
                        Json(
                            {
                                **(getattr(event, "hashes", {}) or {}),
                                "event_hash": getattr(
                                    event,
                                    "event_hash",
                                    (getattr(event, "hashes", {}) or {}).get("event_hash", ""),
                                ),
                            }
                        ),
                        Json(getattr(event, "integrity", {})),
                        getattr(event, "integrity_hash", None)
                        or getattr(
                            event,
                            "event_hash",
                            (getattr(event, "hashes", {}) or {}).get("event_hash", ""),
    ...
    def list(
        self, before: Optional[str] = None, limit: int = 100, filters: Any = None
    ) -> EventPage:
        """
